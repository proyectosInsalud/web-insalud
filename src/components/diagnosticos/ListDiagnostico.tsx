  ...
  return (
    <section className="max-w-7xl container mx-auto px-4 py-8 md:py-16">
      <Tabs defaultValue="disfuncion-erectil">
        ...
        {categories.map((c) => (
          <TabsContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 py-12 font-in-poppins" key={c.value} value={c.value}>
            {tratamientos
              .filter((tratamiento) => tratamiento.categoria === c.value)
              .map((tratamiento) => (
                <div className="border border-[#CCD1EE] p-4 rounded-lg" key={tratamiento.slug}>
                  <Image
